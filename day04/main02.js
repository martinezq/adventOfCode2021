...
do {
    ...
    const roundWinners = game.filter(isWinner);
    
    winners = winners.concat(roundWinners);
    game = game.filter(x => roundWinners.find(w => x === w) === undefined);      
    
    index++;

    console.log(winners.length, game.length);

} while (winners.length < boards.length);

const numberWon = numbers[index-1];
const boardScore = calculateBoardScore(winners.pop());

console.log(index - 1, numberWon, boardScore, numberWon * boardScore);
